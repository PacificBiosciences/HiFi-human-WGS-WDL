...
import "wdl-common/wdl/tasks/write_ped_phrank_parentless.wdl" as Write_ped_phrank
import "tertiary/tertiary_parentless.wdl" as TertiaryAnalysis
...
# 2) Tumor‐only Severus for samples without parental BAMs 
    call Somatic_calling.Severus_sv_wt_only as phased_severus {
       input:
         tumor_bam        = downstream.merged_haplotagged_bam[0],
         tumor_bam_index  = downstream.merged_haplotagged_bam_index[0],
         trf_bed          = trf_bed,
         phased_vcf       = downstream.phased_small_variant_vcf[0],
         threads          = 32,
         min_supp_reads   = 3,
         PON_tsv          = severus_pon_tsv
    }
    call Somatic_calling.tabix_vcf {
      input:
           vcf        = select_first([phased_severus.output_vcf]),
           contig_bed = ref_bed,
           threads    = 32
    } 
    call Somatic_calling.svpack_filter_annotated {
      input:
    ...
    call Somatic_calling.recover_mate_bnd {
      input:
  ...
  output {
    ...
    #Somatic SV calling 
    File? Severus_somatic_vcf                           = phased_severus.output_vcf
    File? Severus_all_vcf                               = phased_severus.output_all_vcf
    File? Severus_breakpoint_cluster                    = phased_severus.output_breakpoint_clusters
    File? Severus_breakpoint_cluster_all                = phased_severus.output_breakpoint_clusters_all
    File? Severus_cluster_plots                         = phased_severus.output_somatic_sv_plots

    File? Severus_tabix_vcf                             = tabix_vcf.output_vcf
    File? Severus_filtered_vcf = recover_mate_bnd.output_vcf
    File? Severus_filtered_vcf_index = recover_mate_bnd.output_vcf_index
   ...
   #annotation analysis outputs
    ...
    File? Severus_annotated_tsv                         = annotateSeverusSVfiltered.annotsv_annotated_tsv
    File? annotsv_annotated_tsv_intogen                         = prioritize_sv.annotSV_intogen_tsv
    File? Severus_annotated_tsv_intogen                         = prioritize_Severus.annotSV_intogen_tsv
